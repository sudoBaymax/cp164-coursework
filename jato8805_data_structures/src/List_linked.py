...
class List:
    ...
    def insert(self, i, value):
        ...
        if i <= 0:
            ...
            for _ in range(1, i):
                prev = prev._next
            node = _List_Node(value, prev._next)
            prev._next = node
            self._count += 1
            result = None
        return result
    ...
    def _linear_search_r(self, key):
        ...
        -------------------------------------------------------
        """
        previous, current, index = self._linear_search_r_aux(None, self._front, 0, key)
        return previous, current, index
    
    
    def _linear_search_r_aux(self, previous, current, index, key):
        if current is None:
            res_prev, res_cur, res_idx = None, None, -1
        elif current._value == key:
            res_prev, res_cur, res_idx = previous, current, index
        else:
            res_prev, res_cur, res_idx = self._linear_search_r_aux(current, current._next, index + 1, key)
        
        return res_prev, res_cur, res_idx
    ...
    def find(self, key):
        ...
        -------------------------------------------------------
        """
        _, current, idx = self._linear_search(key)
        if idx == -1:
            value = None
        else:
    ...
    def index(self, key):
        ...
        -------------------------------------------------------
        """
        _, _, idx = self._linear_search(key)
        return idx
    ...
    def clean(self):
        ...
        if self._front is not None:
            ...
            seen.add(self._front._value)
            
            while cur is not None:
                if cur._value in seen:
                    prev._next = cur._next
                    self._count -= 1
                    if prev._next is None:
                        self._rear = prev
    ...
    def split(self):
        ...
        target2 = List()
        if self._front is None:
            result1, result2 = target1, target2
        ...
        for _ in range(n1):
            ...
            if target1._front is None:
                target1._front = target1._rear = node
            else:
        ...
        while self._front is not None:
            ...
            if target2._front is None:
                target2._front = target2._rear = node
            else:
    ...
    def split_alt_r(self):
        ...
        # call the separate helper method (helper will handle empty source)
        self._split_alt_r_aux(even, odd, True)   # start taking the even index (0)
        return even, odd
    
    
    def _split_alt_r_aux(self, even, odd, take_even):
        """
        Separate helper method (recursive). Moves one node per call from
        self to either even or odd. 'take_even' toggles each recursion.
        Parameters:
            even, odd - List objects to receive nodes
            take_even - bool: True -> move to even, False -> move to odd
        """
        # base case: source empty -> stop
        if self._front is None:
            return
    
        # detach the front node from self
        node = self._front
        self._front = node._next
        node._next = None
    
        # update source counters / rear
        self._count -= 1
        if self._front is None:
            self._rear = None
    
        # append node to destination list (even or odd)
        dst = even if take_even else odd
        if dst._front is None:
            dst._front = dst._rear = node
        else:
            dst._rear._next = node
            dst._rear = node
        dst._count += 1
    
        # recursive call (toggle which list receives the next node)
        self._split_alt_r_aux(even, odd, not take_even)
    ...
    def intersection(self, source1, source2):
        ...
        while node is not None:
            _, found_in_s2, _ = source2._linear_search(node._value)
            if found_in_s2 is not None:
                _, found_in_self, _ = self._linear_search(node._value)
                if found_in_self is None:
    ...
    def _intersection_r_aux(self, node, source2):
        if node is None:
            ...
            _, current_in_source2, _ = source2._linear_search(node._value)
            
            if current_in_source2 is not None:
                _, current_in_self, _ = self._linear_search(node._value)
                
                if current_in_self is None:
                    self.append(node._value)
            
            result = self._intersection_r_aux(node._next, source2)
    ...
    def union(self, source1, source2):
        ...
        cur = source1._front
        while cur is not None:
            _, found, _ = self._linear_search(cur._value)
            if found is None:
                self.append(cur._value)
            cur = cur._next
        cur = source2._front
        while cur is not None:
            _, found, _ = self._linear_search(cur._value)
    ...
    def _union_r_aux(self, node):
        if node is None:
            ...
            result = self._union_r_aux(node._next)
    ...
    def combine(self, source1, source2):
        ...
        if self is source1 or self is source2:
            raise ValueError("Cannot combine a list with itself")
    ...
    def radix_sort(a):
        ...
        if a._front is not None:
            ...
            while cur:
                max_num = max(cur._value, max_num)
                cur = cur._next
                
            while max_num // digits_place > 0:
                buckets = [List() for _ in range(10)]
                
                cur = a._front
                while cur:
                    digits = (cur._value // digits_place) % 10
                    buckets[digits].append(cur._value)
                    cur = cur._next
                ...
                for bucket in buckets:
                    cur = bucket._front
                    while cur is not None:
                        newll.append(cur._value)
                        cur = cur._next
                        
                a._front = newll._front
                a._rear = newll._rear
                a._count = newll._count
                # print([[node for node in bucket] for bucket in buckets])
