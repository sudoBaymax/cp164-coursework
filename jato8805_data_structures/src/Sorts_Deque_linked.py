...
class Sorts:
    ...
    def gnome_sort(a):
        """
        -------------------------------------------------------
        Sorts a Deque using the Gnome Sort algorithm (node swapping).
        Uses pot0, pot1, pot2 naming and swaps actual nodes (relinks),
        not just values. Mutates the Deque in-place.
        Use: Sorts.gnome_sort(a)
        -------------------------------------------------------
        Parameters:
            a - a linked Deque of comparable elements (Deque)
        Returns:
        ...
        """
        # trivial cases
        if a._front is None or a._front._next is None:
            return
    
        # start at the first pair: pot1 = first node, pot2 = second node
        pot1 = a._front
        pot2 = pot1._next
    
        while pot2 is not None:
            # in-order -> step right
            if pot1._value <= pot2._value:
                pot1 = pot2
                pot2 = pot2._next
            else:
                # out-of-order -> swap adjacent nodes pot1 and pot2 by relinking
                pot0 = pot1._prev          # may be None
    
                # keep references to nodes for clarity
                n1 = pot1
                n2 = pot2
                after = n2._next
                ...
                # step left: move pot1 to node before the swapped pair (i--)
                # if pot0 exists, that is the new pot1; otherwise start at front
                if pot0 is not None:
                    pot1 = pot0
                    pot2 = pot1._next
                else:
                    # we are at the head, reset to first pair
                    pot1 = a._front
                    pot2 = pot1._next
