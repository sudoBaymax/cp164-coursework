...
class Queue:
    ...
    def remove(self):
        ...
        # get value to return (deep copy to follow node semantics)
        value = deepcopy(self._front._value)

        # advance front
        # if single node, clear both front and rear
    ...
    def _move_front_to_rear(self, source):
        ...
        node = source._front               # node to move
        source._front = node._next         # advance source front

        # if source becomes empty, clear its rear
        if source._front is None:
            source._rear = None

        node._next = None                  # detach node

        # append node to target (self)
        if self._front is None:            # target empty
            self._front = node
            self._rear = node
        else:
            self._rear._next = node
    ...
    def _append_queue(self, source):
        ...
        # if target empty, take source wholesale
        if self._front is None:
    ...
    def combine(self, source1, source2):
        ...
        -------------------------------------------------------
        """
        # alternate taking nodes from source1 and source2
        turn = 1
        while source1._front is not None or source2._front is not None:
            if turn == 1 and source1._front is not None:
                self._move_front_to_rear(source1)
            elif turn == 2 and source2._front is not None:
