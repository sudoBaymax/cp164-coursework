...
class Priority_Queue:
    ...
    def insert(self, value):
        ...
        if self._front is None:
            ...
            self._rear = node
            inserted = True
        elif self._front._value > node._value:
            ...
            while cur is not None and not inserted: # while it is false
                if prev._value <= node._value <= cur._value:
                    node._next = cur
                    prev._next = node
                    inserted = True
                prev = cur
            ...
            if inserted is False: # if it is false and we reached the end
        ...
        if self._front is None:
            ...
            self._front = node
            self._rear = node
        elif node._value < self._front._value:
            # Insert at front (higher priority)
            node._next = self._front
            self._front = node
        else:
    ...
    def split_key(self, key):
        ...
        -------------------------------------------------------
        """
        # According to convention: lower numeric value => higher priority.
        # So "priority higher than key" means value < key.
        target1 = Priority_Queue()
        target2 = Priority_Queue()
    ...
    def combine(self, source1, source2):
        ...
        -------------------------------------------------------
        """
        # Merge by repeatedly taking the smaller (higher-priority) front value.
        while source1._front is not None and source2._front is not None:
            # If equal, take from source1 first per specification.
            if source1._front._value <= source2._front._value:
                self._move_front_to_rear(source1)
            else:
                self._move_front_to_rear(source2)

        # Append remaining nodes from the non-empty source
    ...
    def _append_queue(self, source):
        ...
        # Case 1: target empty -> take source outright
        if self._front is None:
            ...
            # Case 2: simple concatenation keeps order if all source elements
            # come after target's rear
            if self._rear._value <= source._front._value:
                ...
                # Case 3: need to merge to preserve sorted order
                merged = Priority_Queue()
                # merge fronts until one source empties
                while self._front is not None and source._front is not None:
                    if self._front._value <= source._front._value:
                        merged._move_front_to_rear(self)
                    else:
                        merged._move_front_to_rear(source)
    
                # append the remainder (one of these will be empty)
                if self._front is not None:
                    merged._append_queue(self)
                if source._front is not None:
                    merged._append_queue(source)
    
                # transfer merged into self
                self._front = merged._front
                self._rear = merged._rear
                self._count = merged._count
    
                # merged._append_queue emptied the originals; ensure source empty
                source._front = None
                source._rear = None
                source._count = 0
    ...
    def _move_front_to_rear(self, source):
        ...
        # if source becomes empty, clear its rear
