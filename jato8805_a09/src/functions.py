...
def insert_words(fv, hash_set):
    """
    -------------------------------------------------------
    Retrieves every Word in file_variable and inserts into a Hash_Set.
...
def comparison_total(hash_set):
    ...
    for word_obj in hash_set:
        total += word_obj.comparisons
        if max_word is None or word_obj.comparisons > max_word.comparisons:
            max_word = word_obj
